...
auto_stash() {
    if has_local_changes; then
        log "Detectadas mudanças locais não commitadas. Fazendo stash..."
        STASH_NAME="auto-stash-before-update-$(date +%s)"
        if git stash push -m "$STASH_NAME" > /dev/null 2>&1; then
            success "Stash criado: $STASH_NAME"
            echo "$STASH_NAME"
            return 0
        else
            error "Falha ao criar stash"
            return 1
        fi
    fi
    echo ""
...
restore_stash() {
    ...
    if [ -n "$stash_name" ]; then
        log "Restaurando stash: $stash_name"
        if git stash pop > /dev/null 2>&1; then
            success "Stash restaurado com sucesso"
            return 0
        else
            warn "Falha ao restaurar stash automaticamente. Conflitos podem ter ocorrido."
            warn "Verifique manualmente com: git stash list e git stash pop"
            return 1
        fi
    fi
    return 0
