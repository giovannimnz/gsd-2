 ...
 * Accepts any URL-safe token shape (not only hex) so custom deployments
 * can provide static/shared bearer tokens.
 */
...
export function getAuthToken(): string | null {
  ...
  const hashToken = extractAuthTokenFromHash(window.location.hash)
  if (hashToken) {
    cachedToken = hashToken
...
/**
 * Wrapper around `fetch()` that automatically injects the auth token when
 * available. When no token exists, it performs a normal unauthenticated
 * request so token-optional deployments can still function.
 */
