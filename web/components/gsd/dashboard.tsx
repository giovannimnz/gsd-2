...
import { type WorkspaceTaskTarget } from "@/lib/workspace-types"
...
export function Dashboard({ onSwitchView, onExpandTerminal }: DashboardProps = {}) {
  ...
  const doneTasks = currentSlice?.tasks.filter((t: WorkspaceTaskTarget) => t.done).length ?? 0
              ...
              <div className="flex-1 p-3">
                {currentSlice && currentSlice.tasks.length > 0 ? (
                  <div className="space-y-0.5">
                    {currentSlice.tasks.map((task: WorkspaceTaskTarget) => {
